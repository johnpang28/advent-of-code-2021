import Day08.getWithScrambledKey
import Day08.input
import Day08.parseLine
import Day08.signalToNumber

fun main() {

    val signalOutputPairs = input.map { parseLine(it) }

    val answer1 = signalOutputPairs.sumOf { (_, output) -> output.count { listOf(2, 4, 3, 7).contains(it.length) } }
    println(answer1) // 409

    val answer2 = signalOutputPairs.sumOf { (signals, outputs) ->
        outputs.map { signalToNumber(signals).getWithScrambledKey(it) }.joinToString("").toInt()
    }
    println(answer2) // 1024649
}
